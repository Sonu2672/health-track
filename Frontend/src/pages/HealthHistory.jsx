...
function HealthHistory() {
  ...
  const getRiskLevel = (record) => {
    const heartRate = Number(record.heartRate ?? 0);
    ...
    const envtemp = Number(record.envtemp ?? 0);
    const humidity = Number(record.humidity ?? 0);
    const ecg = Number(record.ecg ?? 0);
    const dust = Number(record.dust ?? 0);
