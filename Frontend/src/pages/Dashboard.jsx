  ...
  useEffect(() => {
    ...
    const Hdata = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Online API Error: ${response.status}`);
        }

        const onlineData = await response.json();

        if (!onlineData || !onlineData.hd) {
          throw new Error("Invalid online health data");
        }
      ...
      // 2. If ONLINE fails, use ESP32 local API
      catch (onlineError) {
        console.log("🌐 Online unavailable → trying ESP32 local API...");
        ...
        try {
          ...
          if (!localResponse.ok) {
            throw new Error(`ESP32 API Error: ${localResponse.status}`);
          }
          ...
          console.log("🟢 ESP32 OFFLINE DATA:", esp32);
        }

        catch (localError) {
          console.error("❌ Both Online and ESP32 APIs unavailable");
          console.error("Online:", onlineError);
          console.error("ESP32:", localError);
      ...
      if (
        ...
        newRiskLevel = "Critical Risk";
      } else if (newRiskLevel === "Warning") {
      ...
      // TinyML confidence 0–1 → dashboard 0–100
      if (
        newRiskScore > 0 &&
        newRiskScore <= 1
      ) {
        newRiskScore =
          Math.round(newRiskScore * 100);
      }

      newRiskScore = Math.max(
        0,
        Math.min(100, newRiskScore)
      );
      ...
      console.log(
        source === "ONLINE"
          ? "🌐 Dashboard Source: ONLINE"
          : "🟢 Dashboard Source: ESP32 OFFLINE"
      );
    };
