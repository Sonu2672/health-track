...
function AppRoutes() {
  ...
  // 1. OneSignal Initialization & Player ID sync
  useEffect(() => {
    const initOneSignal = async () => {
      try {
        ...
        // Prompt user for notification permissions if not already granted
        await OneSignal.Slidedown.promptPush();

        // Get Player ID and send to backend if user is logged in
        setTimeout(async () => {
          if (OneSignal.User && OneSignal.User.PushSubscription) {
            ...
            if (playerId) {
              ...
              try {
                ...
                console.error("Error sending Player ID to backend:", err);
              }
            }
          }
        }, 4000);
